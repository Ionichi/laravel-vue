    ...
    <main>
        ...
        <section class="container mb-1">
            <div class="py-5 d-flex align-items-center justify-content-between">
                <span class="h1 pb-2 border-bottom border-3 border-dark">Tambah Data Course</span>
                <button type="button" @click="resetForm" class="btn btn-secondary text-light">Reset</button>
            </div>
        </section>
        <section class="container pb-5">
            <form @submit.prevent="sendData" class="row needs-validation" novalidate id="formCourse">
                <div class="col-md-6">
...
export default {
    ...
    async created() {
        ...
        try {
            const response = await axios.get('/course/get-tutor');
    ...
    methods: {
        async sendData(event) {
            ...
            if(!form.checkValidity()) {
                ...
                try {
                    const response = await axios.post('/course/create', this.course);
